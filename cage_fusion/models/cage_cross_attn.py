...
class CAGEFusionModel(nn.Module):
    ...
    def forward(
        ...
        if self.use_co_attention and sequence_embeddings is not None:
            ...
            # New Try Oct 3 2025: Only mask PAD tokens
            # mask only PAD (keep CLS and SEP available to attention)
            # pad_mask = (input_ids_batch == self.PAD_TOKEN_ID)           # [B, T], True = ignore
            # if attn_mask is not None:
            #     # combine with HF attention_mask if provided (1=keep, 0=pad)
            #     pad_mask = pad_mask | (attn_mask == 0)

            # # zero out PAD token embeddings for stability; keep CLS/SEP untouched
            # embedding_proj = embedding_proj.masked_fill(pad_mask.unsqueeze(-1), 0.0)

            # # This is the mask MultiheadAttention expects: True = positions to ignore
