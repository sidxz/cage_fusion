"""
cage_fusion/utils/device_utils.py
...
def move_bmg_to_device(bmg: BatchMolGraph, device: torch.device) -> BatchMolGraph:
    """Transfer a :class:`~chemprop.data.BatchMolGraph` to ``device`` in-place."""
...
def compute_pos_weight_from_h5(
    ...
    """
    Compute per-class positive weights from an HDF5 labels dataset.

    Suitable for passing to ``torch.nn.BCEWithLogitsLoss(pos_weight=...)``.

    Args:
        h5_path: Path to HDF5 file containing a ``labels`` dataset of shape
                 ``[N, num_classes]``.
        chunk_size: Row chunk size for streaming.
        epsilon: Smoothing to avoid division by zero.
        verbose: Log per-class counts.
