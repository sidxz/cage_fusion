...
def train_model(
    ...
    """
    Full training loop with logging, per-epoch checkpointing,
    ...
    logger.info(
        f"Batch sizes - Train: {len(train_loader)}, Validation: {len(val_loader)}"
    )
    logger.info(
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        logger.debug(f"Checkpoint saved for epoch {epoch} at {checkpoint_path}")
        ...
        if val_auc > best_val_auc:
            ...
            logger.info(
                f"New best model saved with Val AUC: {best_val_auc:.4f} at {best_model_path}"
            )
