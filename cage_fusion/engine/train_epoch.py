...
def train_one_epoch(
    ...
    mcc_agg = MCCBatchAggregatorToDisk(num_tasks, f"{cache_dir}/mcc_train", label_names)
    auc_agg = AUCBatchAggregatorToDisk(num_tasks, f"{cache_dir}/auc_train", label_names)
    pr_agg = PRBatchAggregatorToDisk(num_tasks, f"{cache_dir}/pr_train", label_names)

    batch_to_log_idx = -1
    if log_attn_random_batch and len(loader) > 0:
        batch_to_log_idx = random.randint(0, len(loader) - 1)
        logger.info(f"Attention visualization for batch index: {batch_to_log_idx}")
    ...
    for batch_idx, batch_data in enumerate(tqdm(loader, desc="Training")):
        ...
        if model_output is None:
            raise ValueError(f"Model returned None on batch {batch_idx}")
    ...
    logger.info(f"Epoch training complete. Avg Loss: {avg_loss:.4f}, MCC: {avg_mcc:.4f}, AUC: {avg_auc:.4f}, PR-AUC: {avg_pr:.4f}")
